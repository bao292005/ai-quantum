-- luna.sql — reproduces fixtures/backtest/luna_2022_05_09.csv.gz
--
-- LUNA/UST depeg window. Block range 14,724,000 → 14,740,000
-- (from_block gives >=24h runway before the first Aave V2 liquidation @ block 14731270).
...
-- The canonical fixture was produced by tools/extract_fixtures.py against the
-- Etherscan V2 logs API; this query is the reproducible SQL equivalent and
-- returns the exact same 11-column tick-data schema (Story 0.1), same ordering
-- (block_number, log_index).
--
-- Run:
--   1. Open https://dune.com/queries → New query.
--   2. Paste this file, Run, then Export → CSV.
--   3. Rename to luna_2022_05_09.csv and gzip (see queries/README.md).
...
WITH univ3_swaps AS (
    ...
    WHERE contract_address = 0x88e6A0c2dDD26FEEb64F039a2c41296FcB3f5640  -- USDC/WETH 0.05%
      AND evt_block_number BETWEEN 14724000 AND 14740000
),
...
aave_borrow AS (
    SELECT
        ...
        0x7d2768dE32b0b80b7a3454c06BdAc94A69DDc7A9 AS pool_address,     -- Aave V2 LendingPool
        reserve    AS token0,
    ...
    WHERE evt_block_number BETWEEN 14724000 AND 14740000
),
...
SELECT
    block_number,
    -- schema Story 0.1 requires ISO 8601 UTC ending with Z
